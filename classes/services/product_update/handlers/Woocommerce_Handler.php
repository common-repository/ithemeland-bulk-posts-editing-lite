...
class Woocommerce_Handler implements Handler_Interface
{
    ...
    private function set_product_regular_price($value)
    {
        if ($value <= $this->product->get_sale_price()) {
            $this->product->set_sale_price(0);
        }
        $this->product->set_regular_price(esc_sql($value));
        $this->product->save();
    }
    ...
    {
        $value = ($value >= $this->product->get_regular_price()) ? floatval($this->product->get_regular_price() - 0.01) : esc_sql($value);
        $this->product->set_sale_price(esc_sql($value));
